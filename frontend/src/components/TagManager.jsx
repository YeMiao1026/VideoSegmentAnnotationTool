...
export default function TagManager({ tags, onTagsUpdate }) {
  ...
  return (
    <div>
      <div style={{ marginBottom: 8, display: 'flex', gap: 8, alignItems: 'center' }}>
        <input value={input} onChange={e => setInput(e.target.value)} placeholder="新增標籤" style={{ flex: 1 }} />
        <button className="btn btn-primary btn-small" onClick={addTag} title="新增標籤">
      ...
      <div>
        {tags.map(t => (
          <div key={t} style={{ display: 'flex', alignItems: 'center', marginBottom: 8 }}>
            ...
            <button className="btn icon-btn" onClick={() => removeTag(t)} title={`刪除 ${t}`}>
